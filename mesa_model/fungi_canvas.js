const ContinuousVisualization = function(width, height, context) {
	this.draw = function(objects) {
		for (const p of objects) {
			...
			if (p.Shape == "line")
				this.drawLine(p.x, p.y, p.size, p.direction, p.w, p.Color)
		};
	...
	this.drawLine = function(x, y, size, direction, w, color) {
		const lx1 = x * width;
		const ly1 = y * height;

		const lx2 = (lx1 + (Math.cos(direction) * size * height))
		const ly2 = (ly1 + (Math.sin(direction) * size * height))
...
const Fungi_Continuous_Module = function(canvas_width, canvas_height) {
	// Create the element
	// ------------------
